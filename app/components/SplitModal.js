...
export default function SplitModal({ expense, existingSplits = [], onSplit, onClose }) {
  ...
  const updateRow = (idx, field, value) => {
    ...
    if (field === "person" && value.length > 0) {
      const filtered = knownPeople.filter((p) => p.toLowerCase().startsWith(value.toLowerCase()));
    ...
    <div className="modal-backdrop" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-card" role="dialog" aria-modal="true" aria-label="Split expense">
        ...
        <div className="modal-body">
          {/* Expense context */}
          <div style={{ background: "var(--bg)", borderRadius: "var(--radius-sm)", padding: "10px 14px", marginBottom: 16, border: "1px solid var(--border)" }}>
            <div style={{ fontWeight: 600, fontSize: 14 }}>{expense.vendor || expense.name}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>
              {expense.date} · {formatCurrency(expense.amount)}
              {alreadySplit > 0 && (
                <span> · Already split: {formatCurrency(alreadySplit)}</span>
              )}
            </div>
